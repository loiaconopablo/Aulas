...
Aulas::App.mailer :notification do

  email :email_notificacion_cuenta do | user |
    from 'no_reply@aulas.com'
    to user.email
    subject 'Aulas: Alta de Usuario'
    locals :user => user
    content_type :plain
  ...
  email :email_reserva_aceptada do | reserva, user |
    from 'no_reply@aulas.com'
    to user.email
    subject 'Aulas: Su reserva aceptada'
    locals :reserva => reserva
    content_type :plain
    render 'notification/email_reserva_aceptada'
  end
